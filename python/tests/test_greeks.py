...
def test_greeks():
    ...
    delta_call = options.delta_wrapper(S, K, T, r, sigma, 1)
    delta_put = options.delta_wrapper(S, K, T, r, sigma, 0)
    print(f"Delta Call: {delta_call}, Delta Put: {delta_put}")

    gamma = options.gamma_wrapper(S, K, T, r, sigma)
    print(f"Gamma: {gamma}")


    theta_call = options.theta_wrapper(S, K, T, r, sigma, 1)
    theta_put = options.theta_wrapper(S, K, T, r, sigma, 0)
    print(f"Theta Call: {theta_call}, Theta Put: {theta_put}")


    vega = options.vega_wrapper(S, K, T, r, sigma)
    print(f"Vega: {vega}")

    rho_call = options.rho_wrapper(S, K, T, r, sigma, 1)
    rho_put = options.rho_wrapper(S, K, T, r, sigma, 0)
